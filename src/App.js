...
function App() {
  const [initialLinks, setInitialLinks] = useState(initialLinkCards);
  ...
  useEffect(() => {
    ...
    if (initialLinks.length !== cardsFromLocalStorage.length) {
      setInitialLinks(cardsFromLocalStorage);
    } else {
      localStorage.setItem("link-cards", JSON.stringify(initialLinkCards));
    }
  }, []);
